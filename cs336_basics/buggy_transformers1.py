# really_hard_bug_transformer_debug.py
# "Really hard" debugging exercise: subtle multi-line bugs that affect attention/residuals/grad flow.
# High-level hint (to give to candidate): bug is in the general area of attention / residual connections / gradient flow.
...
# ---------- Model with subtle, deliberate multi-line bugs ----------
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, attn_mask=None, return_attn=False):
        ...
        # ====== DELIBERATE HARD BUGS (do NOT reveal to candidate) ======
        # (The following two lines are intentionally problematic.)
        #  - They affect backward gradients and subtle runtime behavior.
        #  - Candidate must find and fix them.
        #
        # Note: we keep forward output shape correct so the failure is subtle (gradients/training).
        # v = v.clone().detach()   # <-- subtle: breaks gradient flow to v_proj
        # =================================================================
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        # ====== DELIBERATE HARD BUG (in-place ops) ======
        # Using in-place updates on `x` (x += ...) can interfere with autograd when
        # x is used to compute intermediates required for backward (LayerNorm etc).
        # Candidate must detect and convert to out-of-place additions.
...
# ---------- Scratch CrossEntropy + AdamW (minimal) ----------
...
# ---------- HARD tests that will FAIL while bug present ----------
def grad_flow_test():
    """
    Run one forward/backward step and assert gradients flow to key projection weights.
    With the deliberate bug, some grads will be zero.
    """
    ...
    for k in sorted([k for k in grads.keys() if "q_proj" in k or "k_proj" in k or "v_proj" in k or "head" in k])[:10]:
        print(k, grads[k])

    # Key expectations: q_proj, k_proj, v_proj and head should all have non-zero grads.
    # This will fail while the deliberate bug is present (v_proj will have zero grad, etc.)
...
def attention_mask_test():
    """
    Check that attention matrix respects causal mask (simple sanity).
    """
    model = TinyDecoderTransformer(vocab_size=V, max_len=SEQ_LEN, d_model=64, n_heads=4, n_layers=2, d_ff=256).to(device)
    x = make_batch(batch_size=1, seq_len=6)
    logits, attn_mats = model(x, return_attn=True)
    # attn_mats is a list of per-layer attn tensors (B, heads, L, L)
    attn0 = attn_mats[0]  # first layer
    # check upper triangular entries for pos 0 -> positions 4,5 should be near zero
...
def train_smoke_test():
    """
    Short training run (N_ITERS small) — ensure loss decreases modestly.
    With bug present training may not make progress.
    """
    model = TinyDecoderTransformer(vocab_size=V, max_len=SEQ_LEN, d_model=64, n_heads=4, n_layers=2, d_ff=256).to(device)
    ...
    # expect final loss to be lower than initial
    assert losses[-1] < losses[0], "Loss did not decrease during short training (bug likely)"

# ---------- Training harness (full run as requested) ----------
def run_demo_train():
...
if __name__ == "__main__":
    print("1) Running grad_flow_test (expected to FAIL while bug present)...")
    try:
        grad_flow_test()
    ...
    print("\n3) Running train_smoke_test (short training — expected to fail or show no improvement while bug present)...")
    try:
        train_smoke_test()
        print("train_smoke_test passed.")
    except AssertionError as e:
        print("train_smoke_test failed as expected. Error:", e)

    print("\n4) Optional long demo (N_ITERS=200).")
    run_demo_train()
    print("Done.")