...
def run_swiglu(
    ...
    from cs336_basics.nn import PositionwiseFeedForward
    swiglu = PositionwiseFeedForward(d_model=d_model, d_ff=d_ff)
    # torch.nn.Module.load_state_dict(swiglu, {"w1.weight": w1_weight, "w2.weight": w2_weight, "w3.weight": w3_weight})
...
def run_multihead_self_attention(
    ...
    """
    # max_seq_length, theta, device = None, dtype=None
...
def run_multihead_self_attention_with_rope(
    ...
    Args:
        ...
        v_proj_weight (Float[Tensor, "d_k d_in"]): Weights for the V projection
        o_proj_weight (Float[Tensor, "d_model d_v"]): Weights for the output projection
        in_features (Fweightloat[Tensor, "... sequence_length d_in"]): Tensor to run your implementation on.
...
def run_rope(
    ...
    rope = Rope(theta=theta, d_k=d_k, max_seq_len=max_seq_len, device=None)
...
def run_transformer_block(
    ...
    TransformerBlock.RMSNorm1.param.data = weights["ln1.weight"]
    TransformerBlock.RMSNorm2.param.data = weights["ln2.weight"]
    return TransformerBlock.forward(X=in_features)
...
def run_transformer_lm(
    ...
    for num_layer in range(num_layers):
        ...
        TransformerLM.layers[num_layer].FFN.w3.weight.data = weights[f"layers.{num_layer}.ffn.w3.weight"]
        TransformerLM.layers[num_layer].RMSNorm1.param.data = weights[f"layers.{num_layer}.ln1.weight"]
        TransformerLM.layers[num_layer].RMSNorm2.param.data = weights[f"layers.{num_layer}.ln2.weight"]
    TransformerLM.final_RMSNorm.param.data = weights["ln_final.weight"]
    TransformerLM.output_layer.weight.data = weights["lm_head.weight"]

    return TransformerLM.forward(X = in_indices)
...
def run_rmsnorm(
    ...
    torch.nn.Module.load_state_dict(rmsnorm, {"param": weights})
...
def run_get_batch(
    ...
    """
    from cs336_basics.optimizer import get_batch
    return get_batch(dataset, batch_size=batch_size, context_len=context_length, device=device)
    # raise NotImplementedError
...
def run_cross_entropy(
    ...
    return cross_entropy_loss(inputs, targets=targets)
    # raise NotImplementedError
